...
from bank_guarantee_tool.src.loaders.excel_loader import (
    build_sheet_diagnostics,
    inspect_workbook,
    load_excel_file,
    prepare_guarantees_from_sheets,
    prepare_limits_from_sheets,
)
...
def process_workbook(file_bytes: bytes) -> dict[str, pd.DataFrame | str]:
    ...
    workbook_inventory = inspect_workbook(file_bytes)
    sheet_diagnostics = build_sheet_diagnostics(sheets)
...
def empty_state() -> dict[str, pd.DataFrame | str]:
    return {
        ...
        "sheet_diagnostics": pd.DataFrame(),
    }
...
def render_import_diagnostics(
    ...
    sheet_diagnostics_df: pd.DataFrame,
) -> None:
    st.header("Диагностика импорта Excel")
    st.caption("Здесь видно, какие листы и строки были найдены, какие строки обработаны, а какие требуют проверки.")

    if workbook_inventory_df.empty and guarantees_df.empty:
        st.info("Загрузите Excel-файл, чтобы увидеть диагностику импорта.")
        return

    if not workbook_inventory_df.empty:
        st.subheader("Все листы книги, включая скрытые")
        st.dataframe(workbook_inventory_df, use_container_width=True, hide_index=True)

    if not sheet_diagnostics_df.empty:
        st.subheader("Распознавание листов и строк заголовков")
        st.dataframe(sheet_diagnostics_df, use_container_width=True, hide_index=True)

    if guarantees_df.empty:
        st.warning("Строки банковских гарантий не распознаны.")
        return

    status_counts = guarantees_df["processing_status"].value_counts(dropna=False) if "processing_status" in guarantees_df else pd.Series(dtype=int)
    col1, col2 = st.columns(2)
    col1.metric("Обработано строк", int(status_counts.get("processed", 0)))
    col2.metric("Требуют проверки", int(status_counts.get("needs_review", 0)))

    processed = guarantees_df[guarantees_df.get("processing_status", "") == "processed"]
    needs_review = guarantees_df[guarantees_df.get("processing_status", "") != "processed"]

    st.subheader("Обработанные строки")
    st.dataframe(processed, use_container_width=True, hide_index=True)

    st.subheader("Строки, которые нужно проверить")
    if needs_review.empty:
        st.success("Нет строк, требующих ручной проверки.")
    else:
        st.warning("Эти строки были прочитаны из Excel, но приложение распознало недостаточно ключевых полей.")
...
def main() -> None:
    ...
    workbook_inventory_df = data["workbook_inventory"]
    sheet_diagnostics_df = data["sheet_diagnostics"]
    ...
    page = st.sidebar.radio(
        "Раздел",
        ["Dashboard", "Import Diagnostics", "Bank Limits", "Guarantees", "Suppliers", "Export"],
    )

    if page == "Dashboard":
        render_dashboard(guarantees_df, bank_limits_df, CONFIG.calculation_date)
    elif page == "Import Diagnostics":
        render_import_diagnostics(guarantees_df, workbook_inventory_df, sheet_diagnostics_df)
